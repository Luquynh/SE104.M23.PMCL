...
namespace PhanMemQLTV
{
    public partial class frmBaoCaoThongKe : Form
    {
        ...
        private void slMuon()
        {
            string strTinhSLMuon = "select sum(SLMuon) as Tong from tblPhieuMuon";
            myConnection = new SqlConnection(strKetNoi);
            myConnection.Open();
        ...
        {
            string strTinhSLMuon = "select (count(distinct(MaDG))) as TongSLDGMuon from tblPhieuMuon";
            myConnection = new SqlConnection(strKetNoi);
            myConnection.Open();
            myCommand = new SqlCommand(strTinhSLMuon, myConnection);
        ...
        {
            string strTinhSLSachQuaHan = "SELECT count(SLMuon) as TongSLQuaHan from tblPhieuMuon where CONVERT (datetime, NgayhenTra, 103) < getdate()";
            myConnection = new SqlConnection(strKetNoi);
            myConnection.Open();
        ...
        {
            string strTinhSLDGQuaHan = "SELECT count(distinct(MaDG)) as TongSLQuaHan from tblPhieuMuon where CONVERT (datetime, NgayhenTra, 103) < getdate()";
            myConnection = new SqlConnection(strKetNoi);
            myConnection.Open();
        ...
        private void btnXemSLSachQuaHan_Click(object sender, EventArgs e)
        {
            ...
            string strTimSLSQuaHan = @"SELECT pm.MaPhieu as 'Mã Phiếu', MaDG as 'Mã DG', MaSach as 'Mã Sách', NgayMuon as 'Ngày Mượn', ct.NgayhenTra as 'Ngày Hẹn Trả', GhiChu as 'Ghi Chú' from tblPhieuMuon as pm,ChiTietPM as ct where pm.MaPhieu=ct.MaPhieu and CONVERT (datetime, ct.NgayhenTra, 103) < getdate()";
        ...
        private void btnBaocao_Click(object sender, EventArgs e)
        {
            ...
            string query = "select TheLoai as [Thể loại], sum(SLMuon) as [Số lần mượn] from tblSach S, ChiTietPM CT,tblPhieuMuon as PM where S.MaSach = CT.MaSach and CT.MaPhieu=PM.MaPhieu and month(PM.NgayMuon) = " + dtpBaocao.Value.Month + "and year(PM.NgayMuon) = " + dtpBaocao.Value.Year + " group by S.TheLoai";
        ...
        {
            dtpBaocao.Enabled = false;
            string strTimSLDGQuaHan = @"SELECT MaDG as 'Mã DG', sum(SLMuon) as 'SL Sách Mượn' from tblPhieuMuon pm where CONVERT (datetime, NgayhenTra, 103) <= getdate() group by MaDG";
